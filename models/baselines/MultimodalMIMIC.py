...
class TSMixed(nn.Module):
    def __init__(self, args, device, modeltype=None, orig_d_ts=None, orig_reg_d_ts=None, ts_seq_num=None):
        ...
        if self.TS_model == 'LSTM':
            ...
            self.trans_ts_mem = TimeSeriesCnnModel(input_size=self.d_ts, n_filters=self.d_ts, filter_size=self.kernel_size, dropout=self.dropout, length=self.tt_max, n_neurons=self.d_ts, layers=args.layers)
    ...
    def get_network(self, self_type='ts_mem', layers=-1):
        q_seq_len = self.tt_max if self.irregular_learn_emb_ts else self.ts_seq_num
        return TransformerEncoder(embed_dim=self.d_ts, num_heads=self.num_heads, layers=layers, device=self.device, attn_dropout=self.dropout, relu_dropout=self.dropout, res_dropout=self.dropout, embed_dropout=self.dropout, attn_mask=self.attn_mask, q_seq_len=q_seq_len, kv_seq_len=None)
    ...
    def forward(self, x_ts, x_ts_mask, ts_tt_list, labels=None, reg_ts=None):
        if "TS" in self.modeltype:
            if self.Interp:
                ...
                recon_m = torch.Tensor(hold_out(x_ts_mask_interp)).to(self.device)
                proj_x_ts_interp = self.proj_ts_intp(self.c_intp(self.s_intp(x_ts_interp, x_ts_mask_interp, ts_tt_list, recon_m))).permute(2, 0, 1)
            ...
            if self.TS_mixup:
                if self.Interp and not self.irregular_learn_emb_ts and self.reg_ts: proj_x_ts_irg = proj_x_ts_interp
                if self.Interp and self.irregular_learn_emb_ts and not self.reg_ts: proj_x_ts_reg = proj_x_ts_interp

                if self.mixup_level == 'batch':
                    moe_gate = torch.cat([torch.max(proj_x_ts_irg, dim=0).values, torch.max(proj_x_ts_reg, dim=0).values], dim=-1)
                elif self.mixup_level in ['batch_seq', 'batch_seq_feature']:
                    moe_gate = torch.cat([proj_x_ts_irg, proj_x_ts_reg], dim=-1)
            ...
            if self.TS_model == 'CNN':
                proj_x_ts = self.trans_ts_mem(proj_x_ts.permute(1, 2, 0))
            elif self.TS_model == 'LSTM':
                _, (proj_x_ts, _) = self.trans_ts_mem(proj_x_ts)
            else:
                proj_x_ts = self.trans_ts_mem(proj_x_ts)

            last_h_ts = proj_x_ts if self.TS_model == 'CNN' else proj_x_ts[-1]
        ...
        reconloss_interp = recon_loss(x_ts_interp, x_ts_mask_interp, recon_m, recon_interp, self.d_ts) if self.Interp else 0

        if self.task == 'ihm':
            if labels is not None:
                return self.loss_fct1(output, labels) + (reconloss_interp if self.Interp else 0)
            return torch.nn.functional.softmax(output, dim=-1)[:, 1]
        elif self.task == 'pheno':
            if labels is not None:
                return self.loss_fct1(output, labels.float()) + (reconloss_interp if self.Interp else 0)
            return torch.nn.functional.sigmoid(output)